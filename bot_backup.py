...
async def planned_get_time(message: types.Message):
    ...
    if not state or state.get("step") != "planned_get_time":
        return
    text = message.text.strip()
    if text == "⬅ Назад":
        return
...
logging.warning("HIT: manual_from")
async def manual_from(message: types.Message):
    logging.warning(f"HIT: manual_from, text={message.text}")
    uid = message.from_user.id
    if uid not in user_state:
        return
...
logging.warning("HIT: get_from_text")
async def get_from_text(message: types.Message):
    logging.warning(f"HIT: get_from_text, text={message.text}")
    uid = message.from_user.id
    if uid not in user_state:
        return
    if user_state[uid].get("step") != "waiting_from_text" or message.text == "⬅ Назад":
        return
...
logging.warning("HIT: manual_to")
async def manual_to(message: types.Message):
    logging.warning(f"HIT: manual_to, text={message.text}")
    uid = message.from_user.id
    if uid not in user_state:
        return
    if user_state[uid].get("step") == "planned_to_address":
...
logging.warning("HIT: get_to_text")
async def get_to_text(message: types.Message):
    logging.warning(f"HIT: get_to_text, text={message.text}")
    uid = message.from_user.id
    if uid not in user_state:
        return
    if user_state[uid].get("step") != "waiting_to_text" or message.text == "⬅ Назад":
        return
